...
namespace detail {
	...
	{
		uint_4 res;
		res.set_x((condition.get_x())? b.get_x() : a.get_x());
		res.set_y((condition.get_y())? b.get_y() : a.get_y());
		res.set_z((condition.get_z())? b.get_z() : a.get_z());
		res.set_w((condition.get_w())? b.get_w() : a.get_w());
		return res;
	...
	static
    inline
    unsigned int prefixScanVectorEx_for_key(uint_4* data) restrict(amp)
	{
		unsigned int sum = 0;
		unsigned int tmp = data[0].get_x();
		data[0].set_x(sum);
		sum += tmp;
		tmp = data[0].get_y();
		data[0].set_y(sum);
											...
											const concurrency::tiled_index<WG_SIZE>& t_idx) restrict(amp)
	{
		for(int bitIdx=0; bitIdx<BITS_PER_PASS; bitIdx++)
		{
			...
			if(!Asc_sort)
			{
				temp.set_x((cmpResult.get_x() != mask));
				temp.set_y((cmpResult.get_y() != mask));
				temp.set_z((cmpResult.get_z() != mask));
				temp.set_w((cmpResult.get_w() != mask));
			}
			else
			{
				temp.set_x((cmpResult.get_x() != 0));
				temp.set_y((cmpResult.get_y() != 0));
				temp.set_z((cmpResult.get_z() != 0));
				temp.set_w((cmpResult.get_w() != 0));
			}
			prefixSum = SELECT_UINT4_FOR_KEY(uint_4{1,1,1,1},
											 uint_4{0,0,0,0},
			...
			prefixSum = localPrefixSum256V_for_key(prefixSum, lIdx, &total, ldsSortData, t_idx);
			{
				...
				ldsSortData[dstAddr.get_x()] = sortData[0];
				ldsSortData[dstAddr.get_y()] = sortData[1];
				ldsSortData[dstAddr.get_z()] = sortData[2];
				ldsSortData[dstAddr.get_w()] = sortData[3];

				ldsSortVal[dstAddr.get_x()] = sortVal[0];
				ldsSortVal[dstAddr.get_y()] = sortVal[1];
				ldsSortVal[dstAddr.get_z()] = sortVal[2];
				ldsSortVal[dstAddr.get_w()] = sortVal[3];

				t_idx.barrier.wait();

				sortData[0] = ldsSortData[localAddr.get_x()];
				sortData[1] = ldsSortData[localAddr.get_y()];
				sortData[2] = ldsSortData[localAddr.get_z()];
				sortData[3] = ldsSortData[localAddr.get_w()];

				sortVal[0] = ldsSortVal[localAddr.get_x()];
				sortVal[1] = ldsSortVal[localAddr.get_y()];
				sortVal[2] = ldsSortVal[localAddr.get_z()];
				sortVal[3] = ldsSortVal[localAddr.get_w()];
												  ...
												  const concurrency::tiled_index<WG_SIZE>& t_idx) restrict(amp)
	{
		...
		for(int bitIdx=0; bitIdx<BITS_PER_PASS; bitIdx++)
		{
			...
			prefixSum = localPrefixSum256V_for_key( prefixSum, lIdx, &total, ldsSortData, t_idx);
			{
				...
				ldsSortData[dstAddr.get_x()] = signedints[0];
				ldsSortData[dstAddr.get_y()] = signedints[1];
				ldsSortData[dstAddr.get_z()] = signedints[2];
				ldsSortData[dstAddr.get_w()] = signedints[3];

				t_idx.barrier.wait();
				signedints[0] = ldsSortData[localAddr.get_x()];
				signedints[1] = ldsSortData[localAddr.get_y()];
				signedints[2] = ldsSortData[localAddr.get_z()];
				signedints[3] = ldsSortData[localAddr.get_w()];
				t_idx.barrier.wait();
			}
		}
	}
