...
public:
    ...
    */
    ...
    {
    public:
        reference_base( Container& rhs, size_type index ) restrict(cpu, amp)
            : m_Container( rhs, false ), m_Index( index )
        {}

        //  Automatic type conversion operator to turn the reference object into a value_type
        operator value_type( ) const restrict(cpu, amp)
        {
        ...
        reference_base< Container >& operator=( const value_type& rhs ) restrict(cpu, amp)
        {
            arrayview_type av( m_Container.m_devMemory );
    ...
    */
    ...
    {
    public:
        const_reference_base( const Container& rhs, size_type index ) restrict(cpu, amp)
            : m_Container( rhs, false ), m_Index( index )
        {}
        ...
        const_reference_base< const Container >& operator=( const value_type& rhs ) restrict(cpu, amp)
        {
            arrayview_type av( m_Container.m_devMemory );
            av[static_cast< int >( m_Index )] = rhs;
        ...
        /*! \brief A get accessor function to return the encapsulated device_vector.
        */
        const Container& getContainer( ) const restrict(cpu, amp)
        {
            return m_Container;
        }
...
private:
    ...
    */
    ...
    public:
        ...
        reverse_iterator_base( ) restrict(cpu, amp)
            : m_Container( create_empty_array< value_type >::getav( ) ), m_Index( 0 )
        {}

        //  Basic constructor requires a reference to the container and a positional element
        reverse_iterator_base( Container& lhs, int index ) restrict(cpu, amp)
            : m_Container( lhs, false ), m_Index( static_cast<int>(index-1) )
        {}

        //  This copy constructor allows an iterator to convert into a const_iterator, but not vica versa
        template< typename OtherContainer >
        reverse_iterator_base( const reverse_iterator_base< OtherContainer >& lhs ) restrict(cpu, amp)
            : m_Container( lhs.m_Container, false ), m_Index( lhs.m_Index )
        {}

		reverse_iterator_base( const reverse_iterator_base& lhs ) restrict(cpu, amp)
            : m_Container( lhs.m_Container, false ), m_Index( lhs.m_Index )
        {}
        //  This copy constructor allows an iterator to convert into a const_iterator, but not vica versa
        reverse_iterator_base< Container >& operator= ( const reverse_iterator_base< Container >& lhs ) restrict(cpu, amp)
        {
        ...
        reverse_iterator_base< Container >& operator+= ( const difference_type & n ) restrict(cpu, amp)
        {
            advance( -n );
            return *this;
        }

        const reverse_iterator_base< Container > operator+ ( const difference_type & n ) const restrict(cpu, amp)
        {
        ...
        int getIndex() const restrict(cpu, amp)
        {
            return m_Index;
        }
        ...
        difference_type distance_to( const reverse_iterator_base< Container >& lhs ) const restrict(cpu, amp)
        {
        ...
        template< typename OtherContainer >
        bool equal( const reverse_iterator_base< OtherContainer >& lhs ) const restrict(cpu, amp)
        {
    ...
    */
    ...
    device_vector(InputIterator begin, size_type newSize, bool discard = false, control& = control::getDefault( ),
    ...
    */
	template< typename U >
	device_vector( const device_vector<U, CONT> &cont, bool copy = true,control& = control::getDefault( ) ): m_Size( cont.size( ) ),
    ...
    */
    device_vector( arrayview_type &cont) restrict(cpu, amp)
        : m_Size(cont.get_extent().size()), m_devMemory(cont)
    {
    };
    ...
    */
	device_vector( array_type &cont) restrict(cpu, amp)
        : m_Size(cont.get_extent().size()), m_devMemory( cont.view_as(cont.get_extent()))
    {
    };
    ...
    */
    template< typename InputIterator >
    device_vector( const InputIterator begin, const InputIterator end, bool discard = false, control& = control::getDefault( ),
    ...
    */
    ...
    ~device_vector() restrict(cpu, amp)
    {
    }
    ...
    *   this location seems less intrusive to the design of the vector class.
    */
	arrayview_type getBuffer( const_iterator itr, unsigned int size ) const restrict(cpu, amp)
    {
	...
	arrayview_type getBuffer( const_reverse_iterator itr, unsigned int size  ) const restrict(cpu, amp)
    {
		if(size == static_cast<unsigned int>(m_Size))
		{
			hc::extent<1> ext( static_cast< int >( m_Size ) );
    ...
    */
    size_type size( void ) const restrict(cpu, amp)
    {
        return m_Size;
    }
    ...
    */
    iterator begin( void ) restrict(cpu, amp)
    {
    ...
    */
    const_iterator begin( void ) const restrict(cpu, amp)
    {
    ...
    *   \return A device_vector< value_type >::const_iterator.
    */
    const_iterator cbegin( void ) const restrict(cpu, amp)
    {
        return const_iterator( *this, 0 );
    }
    ...
    */
    ...
    reverse_iterator rbegin( void ) restrict(cpu, amp)
    {
    ...
    */
    ...
    const_reverse_iterator rbegin( void ) const restrict(cpu, amp)
    {
    ...
    */
    ...
    const_reverse_iterator crbegin( void ) const restrict(cpu, amp)
    {
        return const_reverse_iterator(*this,m_Size);
    }
    ...
    *   \return A device_vector< value_type >::iterator.
    */
    iterator end( void ) restrict(cpu, amp)
    {
    ...
    *   \return A device_vector< value_type >::const_iterator.
    */
    const_iterator end( void ) const restrict(cpu, amp)
    {
    ...
    *   \return A device_vector< value_type >::const_iterator.
    */
    const_iterator cend( void ) const restrict(cpu, amp)
    {
        return const_iterator( *this, m_Size );
    }
    ...
    */
    ...
    reverse_iterator rend( void ) restrict(cpu, amp)
    {
    ...
    */
    ...
    const_reverse_iterator rend( void ) const restrict(cpu, amp)
    {
    ...
    */
    ...
    const_reverse_iterator crend( void ) const restrict(cpu, amp)
    {
        return const_reverse_iterator( *this, 0  );
    }
    ...
    */
    value_type& front( void ) restrict(cpu, amp)
    {
		return (*begin());
    }
    ...
    *   \return Returns a const_reference, which is not a proxy object.
    */
    const value_type& front( void ) const restrict(cpu, amp)
    {
        return (*begin());
    }
    ...
    *   \note This returns a proxy object, to control when device memory gets mapped.
    */
    value_type& back( void ) restrict(cpu, amp)
    {
		return (*(end() - 1));
    }
    ...
    *   \return Returns a const_reference, which is not a proxy object.
    */
    const value_type& back( void ) const restrict(cpu, amp)
    {
		return ( *(end() - 1) );
    }
    ...
    pointer data( void ) restrict(cpu, amp)
    {
    ...
    const_pointer data( void ) const restrict(cpu, amp)
    {
		if(0 == size())
        {
             return NULL;
    ...
    */
    bool empty( void ) const restrict(cpu, amp)
    {
    ...
    */
    ...
    {
        if( m_Size > capacity( ) )
            throw std::runtime_error{"device_vector size can not be greater than capacity( )"};
    ...
    */
    void pop_back( void ) restrict(cpu, amp)
    {
        if( m_Size > 0 )
        {
            --m_Size;
    ...
    */
    iterator erase( const_iterator index )
    {
        ...
        if( index.m_Index >= l_End.m_Index )
            throw std::runtime_error{"Iterator is pointing past the end of this container"};
    ...
    */
	template<typename InputIterator>
    typename std::enable_if<std::is_integral<typename std::iterator_traits<InputIterator>::difference_type>::value>::type
